-- Enhanced Supabase Schema Setup for TeleMed Application (FINAL FIXED VERSION)
-- Run this in your Supabase SQL editor
-- This version resolves foreign key constraint type mismatch issues
...
-- Create doctors table with consistent ID generation
...
-- Create appointments table with consistent ID generation
CREATE TABLE public.appointments (
    id TEXT PRIMARY KEY DEFAULT ('apt_' || substr(gen_random_uuid()::text, 1, 8)),
    patient_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
    doctor_id TEXT NOT NULL, -- No foreign key constraint to avoid type mismatch
    doctor_name TEXT,
...
-- Create prescriptions table WITHOUT foreign key constraints to avoid type mismatch issues
CREATE TABLE public.prescriptions (
    id TEXT PRIMARY KEY DEFAULT ('prs_' || substr(gen_random_uuid()::text, 1, 8)),
    appointment_id TEXT, -- Made nullable, removed foreign key constraint
    patient_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
    doctor_id TEXT NOT NULL, -- Removed foreign key constraint
...
-- Insert sample doctor data with new ID format
...
('doc_001', 'Dr. Rajesh Kumar', 'rajesh.kumar@hospital.com', '+91-9876543210', 'General Medicine', 'MBBS, MD', 'MED001', 15, 500.00, '{"en", "hi"}', 'Experienced general physician with expertise in treating common ailments and preventive care.'),
('doc_002', 'Dr. Priya Sharma', 'priya.sharma@hospital.com', '+91-9876543211', 'Cardiology', 'MBBS, DM Cardiology', 'CARD001', 12, 800.00, '{"en", "hi"}', 'Specialist in heart diseases and cardiovascular interventions.'),
('doc_003', 'Dr. Amit Singh', 'amit.singh@hospital.com', '+91-9876543212', 'Pediatrics', 'MBBS, MD Pediatrics', 'PED001', 8, 600.00, '{"en", "hi", "pa"}', 'Child specialist with focus on infant and adolescent health.'),
('doc_004', 'Dr. Sunita Devi', 'sunita.devi@hospital.com', '+91-9876543213', 'Gynecology', 'MBBS, MS Gynecology', 'GYN001', 10, 700.00, '{"en", "hi", "pa"}', 'Women health specialist providing comprehensive gynecological care.'),
('doc_005', 'Dr. Harjeet Kaur', 'harjeet.kaur@hospital.com', '+91-9876543214', 'Dermatology', 'MBBS, MD Dermatology', 'DERM001', 6, 550.00, '{"en", "pa"}', 'Skin specialist treating various dermatological conditions.')
ON CONFLICT (id) DO NOTHING;
...
COMMENT ON TABLE public.prescriptions IS 'Prescription details from doctor consultations - Uses application-level referential integrity instead of foreign keys';
...
COMMENT ON COLUMN public.prescriptions.doctor_id IS 'References doctors.id but without foreign key constraint to avoid type mismatch issues';
COMMENT ON COLUMN public.prescriptions.appointment_id IS 'References appointments.id but without foreign key constraint to avoid type mismatch issues';
...
SELECT 'Database schema created successfully! All foreign key constraint issues have been resolved.' AS status;